...
from matchnet.inference import MatchNetAssign
from matchnet.generate_data import GeneratePairs
...
class CLRHead(nn.Module):
    def __init__(self,
        ...
        if not self.cfg.train_matchnet: 
            self.matchnet_assign = MatchNetAssign(self.cfg)
        else:
            self.matchnet_batch_size = self.cfg.matchnet_batch_size
    ...
    def loss(self,
        ...
        if self.cfg.train_matchnet:
            sel_gt_ind_batch = torch.zeros((self.matchnet_batch_size,1,1)).cuda()
            pair_features_batch = torch.zeros((self.matchnet_batch_size,1,6)).cuda()
            pair_match_batch = torch.zeros((self.matchnet_batch_size,1,1)).cuda()
        ...
        for stage in range(self.refine_layers):
            ...
            for predictions, target in zip(predictions_list, targets):
                ...
                # label assignment                
                with torch.no_grad():
                    if self.cfg.train_matchnet: # classic cost function assign
                        matched_row_inds, matched_col_inds, cost_input, cost_target, cost_gt, cost_lane_idx = assign(
                                     predictions, target, self.img_w, self.img_h, train_matchnet = True, matchnet_batch=self.matchnet_batch_size, generate_data=self.generate_data)
                    ...
                    else: # matchnet assign
                        self.matchnet_assign.model.eval()
                        matched_row_inds, matched_col_inds = assign(predictions, target, self.img_w, self.img_h, train_matchnet = False, 
                                                                    matchnet_assign = self.matchnet_assign)                        
