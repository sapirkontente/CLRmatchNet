...
from matchnet.train import TrainMatchNet
...
class Runner(object):
    def __init__(self, cfg):
        ...
        if self.cfg.train_matchnet:
            self.matchnet = TrainMatchNet(self.cfg)
    ...
    def resume(self):
        if not self.cfg.load_from and not self.cfg.finetune_from:
            return
        load_network(self.net, self.cfg.load_from, finetune_from=self.cfg.finetune_from, matchnet_ckp=self.cfg.matchnet_ckp, logger=self.recorder.logger)

    def train_epoch(self, epoch, train_loader):
        self.net.train()
        if self.cfg.train_matchnet: self.matchnet.model.train() 
        end = time.time()
        ...
        if self.cfg.train_matchnet:
            torch.save(self.matchnet.model.state_dict(), f'matchnet/ckp/{epoch}.pth')
